module metaschool::RockPaperScissors_01 {
    ...
    public entry fun createGame(account: &signer) acquires DuelResult {
        if (exists<DuelResult>(signer::address_of(account))) {
            ...
            let result = DuelResult {
                ...
                duel_result: utf8(b"Game not yet played")
            };
    ...
    entry fun duel(account: &signer, user_selection: String) acquires DuelResult {
        ...
        } else if ((user_selection == utf8(b"Rock")
            && *computer_selection == utf8(b"Scissors"))
            || (user_selection == utf8(b"Paper")
                && *computer_selection == utf8(b"Rock"))
            || (
                user_selection == utf8(b"Scissors")
                    && *computer_selection == utf8(b"Paper")
            )) {
